...
#[test]
/// Ensures that the usage code block in the README is the same as the output of jd -h
fn usage_is_the_same() {
    ...
    let mut cmd = Command::cargo_bin("jd").unwrap();
    cmd.arg("-h").assert().stdout(readme_usage);
}

#[test]
/// Ensures that the correct version of jd is found in the README
